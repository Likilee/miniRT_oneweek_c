...
t_color3	ray_color(t_ray *r, t_objects *objs)
{
	...
	if (hit(objs, r, &rec))
	{
		direct_color = direct_light(objs,r, &rec);
		if (objs->emit != NULL)
			vplus(direct_color, objs->emit->light);
		return (vmin(vmult_(direct_color, rec.color), color3(1, 1, 1)));
	}
	else
	{
		//구와 교점이 없으면 하늘로
...
t_color3		direct_light(t_objects *objs, t_ray *r, t_hit_record *rec)
{
	...
	direct_light = color3(0, 0, 0);
	objs_first = objs;
	while(objs)
	{
		if(objs->emit != NULL)
			direct_light = vplus(direct_light, direct_light_get(objs_first, objs->emit, r, rec));
		objs = objs->next;
	}
	// dprintf(2,"direct_light:%f, %f, %f\n", direct_light.x, direct_light.y, direct_light.z);
	return (direct_light);
}
...
t_color3		direct_light_get(t_objects *objs, t_emit *light, t_ray *r, t_hit_record *rec)
{
	...
	double		kd;
	double		spec;
	light_dir = vminus(light->center, rec->p);
	...
	ambient = vmult(light->light, light->ka);
	diffuse = vmult(light->light, kd);
	spec = pow(fmax(vdot(view_dir, reflect_dir), 0.0), light->ksn);
	specular = vmult(vmult(light->light, kd), spec);
	...
	lightradiusmask = pow(saturate(1 - pow((pow(light_len,2) / pow(light->attenuationradius,2)), 2)), 2);
