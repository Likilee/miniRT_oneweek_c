...
int	main(void)
{
	...
	global.ambient = color3(0.3, 0.1, 0.1);
	//Scene setting;
	...
	rotate_cy = rotate(vec3(0,0,30));
	rotate_sq = rotate(vec3(0,-45,0));
	objects = NULL;
	diffuse = material(DIFFUSE, color3(0.4,0.3,0.2), 0);
	...
	oadd(&objects, object(CY, cylinder(point3(-0.0,-0.5,-2),vec3(0,1,0), 1, 1, diffuse), rotate_cy));
	// oadd(&objects, object(LIGHT, light(point3(0, 2.45, -3), color3(6, 6, 6), 0.2), NULL));
	oadd(&objects, object(LIGHT, light(point3(1.5, 0, 0), color3(8, 8, 8), 0.1), NULL));
	...
	oadd(&objects, object(PL, plane(point3(0, 2.5, 0), vec3(0, -1, 0), diffuse), NULL));
	oadd(&objects, object(PL, plane(point3(0, -2.0, 0), vec3(0, 1, 0), diffuse), NULL));
