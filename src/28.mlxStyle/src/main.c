...
int	main(int argc, char **argv)
{
	...
	if (argc == 2)
	{
		...
		// printf("scene width:%d, height:%d, ratio:%f\n",scene->canv.width,  scene->canv.height, scene->canv.aspect_ratio);
		// printf("Ambient: (%f, %f, %f) ",scene->global.ambient.x, scene->global.ambient.y, scene->global.ambient.z);
		// printf("scene width:%d, height:%d, ratio:%f\n",scene->canv.width,  scene->canv.height, scene->canv.aspect_ratio);
		// printf("camera lookfrom:(%f, %f, %f) lookdir(%f,%f,%f), hfov: %f\n",((t_camera *)scene->cam_list->element)->orig.x, ((t_camera *)scene->cam_list->element)->orig.y, ((t_camera *)scene->cam_list->element)->orig.z,
		// 																	((t_camera *)scene->cam_list->element)->dir.x, ((t_camera *)scene->cam_list->element)->dir.y, ((t_camera *)scene->cam_list->element)->dir.z,
		// 																													((t_camera *)scene->cam_list->element)->hfov);
		// // scene->cam_list = scene->cam_list->next;
		// printf("camera lookfrom:(%f, %f, %f) lookdir(%f,%f,%f), hfov: %f\n",((t_camera *)scene->cam_list->element)->orig.x, ((t_camera *)scene->cam_list->element)->orig.y, ((t_camera *)scene->cam_list->element)->orig.z,
		// 																	((t_camera *)scene->cam_list->element)->dir.x, ((t_camera *)scene->cam_list->element)->dir.y, ((t_camera *)scene->cam_list->element)->dir.z,
		// 																													((t_camera *)scene->cam_list->element)->hfov);
		// printf("light point(%f,%f,%f) light(%f,%f,%f)\n", ((t_light *)scene->world->element)->p.x, ((t_light *)scene->world->element)->p.y, ((t_light *)scene->world->element)->p.z,
		// 																((t_light *)scene->world->element)->light_color.x, ((t_light *)scene->world->element)->light_color.y, ((t_light *)scene->world->element)->light_color.z);
		// // scene->world = scene->world->next;
		// printf("Sphere center(%f, %f, %f) radius2(%f)\n", ((t_sphere *)scene->world->element)->center.x, ((t_sphere *)scene->world->element)->center.y, ((t_sphere *)scene->world->element)->center.z, ((t_sphere *)scene->world->element)->radius2);
		...
		mlx = mlx_init();
		mlx_win = mlx_new_window(mlx, scene->canv.width, scene->canv.height, "Kilee's raytracer");
		img.img = mlx_new_image(mlx, scene->canv.width, scene->canv.width);
		img.addr = mlx_get_data_addr(img.img, &img.bits_per_pixel, &img.line_length,
									&img.endian);
		//global option
		scene->global.sample_per_pixel = 100;
		// Render
