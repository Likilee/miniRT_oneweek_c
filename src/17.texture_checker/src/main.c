...
int	main(void)
{
	...
	oadd(&objects, object(SP, sphere(point3(0, 0, -2), 0.5, diffuse, solid), NULL));
	// oadd(&objects, object(SQ, square(point3(0, 0, -5), vec3(0, 0, 1), 5, diffuse), rotate_sq));
	// oadd(&objects, object(CY, cylinder(point3(0.5, -0.5, -2),vec3(0,1,0), 1, 1, diffuse, solid), rotate_cy));
	// oadd(&objects, object(LIGHT, light(point3(0, 2.45, -3), color3(6, 6, 6), 0.2), NULL));
	oadd(&objects, object(LIGHT, light(point3(1, 0, -2), color3(3, 3, 3), 0.1), NULL));
