...
static void		pixel_put_path_trace(t_scene *s, t_ray *r, int *i)
{
	double			uv[2];
	...
	while (++i[2] < s->global.spp)
	{
		...
		pixel_color = vplus(pixel_color,
							ray_color_path_trace(r, s, s->global.depth));
	}
...
void			render_path_trace(t_cntl *cntl)
{
	...
	{
		data[i].lane = i;
		if (pthread_create(&p_thread[i], NULL, render_thread_path,
										(void *)&data[i]) < 0)
		{
			perror("Error\n: ");
			exit(0);
